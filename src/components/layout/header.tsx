  ...
  return (
    <header className="h-14 border-b bg-background flex items-center justify-between px-4">
      <div className="flex items-center gap-3 min-w-0">
        <Database aria-hidden="true" className="h-6 w-6 text-primary shrink-0" />
        <h1 className="text-lg font-semibold shrink-0" translate="no">
          SQLite View
        </h1>
        {isConnected && (
          <span className="text-sm text-muted-foreground ml-2 truncate" translate="no">
            - {fileName}
          </span>
        )}
      </div>
      <div className="flex items-center gap-2">
        ...
        <Button variant="outline" size="sm" onClick={handleOpenFile}>
          <FolderOpen aria-hidden="true" className="h-4 w-4 mr-2" />
